...
object Parser {
    ...
    fun readItem(parser: XmlPullParser): ItemFeed {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            if (name == "title") {
                ...
                downloadLink = readAttributeData(parser, "url")
                //move o ponteiro do parser para o próximo item, caso contrario esse qhile termina
                parser.next()
            } else {
                skip(parser)
            }
    ...
    @Throws(IOException::class, XmlPullParserException::class)
    fun readAttributeData(parser: XmlPullParser, attribute: String): String {
        var result = parser.getAttributeValue(null, attribute)
        return result
    }
